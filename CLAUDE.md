...
- **INV-1007 carries a −$110 grand-total error.** Line items sum to 14,750, tax at 6% is 885, so the total should be 15,635 — the document states 15,525. Note the **direction**: unlike INV-1013's +$50 overcharge, this is an *undercharge*. Both warrant an `AR-` finding, but the business impact differs — an overcharge is a direct loss, an undercharge is a reliability defect that creates a downstream reconciliation problem. `AR-004` should carry the signed delta so the Adjudicator can reason about direction.
- **The corpus contains exactly two grand-total errors** — INV-1013 (+$50) and INV-1007 (−$110) — plus INV-1009's subtotal mismatch (states 1,000 against line items summing to −250). Every other invoice reconciles exactly on both the line-sum and the subtotal + tax + charges steps.
